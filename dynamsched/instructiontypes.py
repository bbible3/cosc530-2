...
class InstructionType:
    def __init__(self, instruction_class: InstructionClass, name: str, description="No Description", num_params:int=-1, uses_memory:bool=False):
...
class InstructionTypes:
    def __init__(self):
        self.types = []
    def add(self, instruction_class: InstructionClass, name: str, description="No Description", num_params:int=-1, uses_memory=False):
        self.types.append(InstructionType(instruction_class, name, description=description, num_params=num_params, uses_memory=uses_memory))
...
class DefaultInstructionTypes:
    def default_instruction_types():
        ...
        instruction_types.add(InstructionClass.DATA_TRANSFERS, "lw", description="Load Word", num_params=2, uses_memory=True)
        instruction_types.add(InstructionClass.DATA_TRANSFERS, "flw", description="Floating Point Load Word", num_params=2, uses_memory=True)
        instruction_types.add(InstructionClass.DATA_TRANSFERS, "sw", description="Store Word", uses_memory=True)
        instruction_types.add(InstructionClass.DATA_TRANSFERS, "fsw", description="Floating Point Store Word", num_params=2, uses_memory=True)
